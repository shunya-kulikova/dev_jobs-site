...
gulp.task('img', function () {
    return gulp.src([
        dirs.source + '/assets/img/*.{gif,png,jpg,jpeg,svg,mp4,webp}',
         // какие файлы обрабатывать (путь из константы, маска имени, много расширений)
    ],
...
gulp.task("copy", () => {
    return gulp.src(dirs.source + '/assets/img//sprite*.svg')
    .pipe(gulp.dest('build' + '/assets/img/sprite/'));
});

gulp.task("sprite", () => {
    return gulp.src("dev/assets/img/sprite/*.svg")
      .pipe(svgstore({
        inlineSvg: true
      }))
  ...
  gulp.task("webp", () => {
    return gulp.src("build/assets/img/*.{png,jpg,jpeg}")
...
gulp.task("vector images", () => {
    return gulp.src("build/assets/img/**/*.svg")
      .pipe(imagemin([
        imagemin.svgo()
      ]))
      .pipe(gulp.dest("build/assets/img"));
...
gulp.task("build", gulp.series(
    "clean",
    gulp.parallel('sass', 'img', 'vector images', 'copy', 'imguploads', 'copyFonts', 'copyCSS', 'copyJS'),
    "sprite",
    'webp',
    'html',
    'php'
...
gulp.task('serve', gulp.series('build', function() {
    browserSync.init({ // запускаем локальный сервер (показ, автообновление, синхронизацию)
        ...
        port: 7777, // порт, на котором будет работать сервер
    ...
    gulp.watch( // следим за SVG
        dirs.source + '/assets/img/sprite/*.svg',
        gulp.series('sprite', 'html', reloader)
    );
    // gulp.watch( // следим за SVG
